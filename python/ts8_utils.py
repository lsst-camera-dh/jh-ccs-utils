...
def write_REB_info(ts8sub, outfile='reb_info.txt'):
    ...
    """
    reb_names = ts8sub.sendSynchCommand(10, 'getREBDeviceNames').getResult()
    fw_vers = ts8sub.sendSynchCommand(10, 'getREBHwVersions').getResult()
    SNs = ts8sub.sendSynchCommand(10, 'getREBSerialNumbers').getResult()
...
def get_REB_info(ts8sub, rebid):
    ...
    rebids = list(ts8sub.sendSynchCommand(10, 'getREBIds').getResult())
    rebids[:] = [x % 4 for x in rebids]
    dev_names = list(ts8sub.sendSynchCommand(10, 'getREBDevices').getResult())
    hw_versions = list(ts8sub.sendSynchCommand(10, 'getREBHwVersions').getResult())
    serial_numbers \
        = ['%x' % x for x in
           ts8sub.sendSynchCommand(10, 'getREBSerialNumbers').getResult()]
...
def set_ccd_info(ccs_sub, ccd_names, logger):
    ...
    geo = ccs_sub.ts8.sendSynchCommand(2, "printGeometry 3").getResult()
    for line in geo.split('\n'):
        ...
        ccdtemp = ccs_sub.ts8.sendSynchCommand(2, command).getResult()
        ...
        hv = ccs_sub.rebps.sendSynchCommand(10, command).getResult()
