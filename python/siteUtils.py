...
class HarnessedJobFilePaths:
    ...
    harnessed jobs.
    """
    def __init__(self, run, user, db_name='Prod', prodServer=False):
        """
        Parameters
        ----------
        run: str
        ...
        user: str
            Operator's userid.
        db_name: str ['Prod']
            Name of the eT database to use.
        prodServer: bool [False]
            Flag to use the prod eT server.  If False, then use the dev
            eT server.
        """
        self.conn = Connection(user, db_name, prodServer=prodServer)
        self.resp = self.conn.getRunFilepaths(run=str(run))
